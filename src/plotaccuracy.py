import pickle
import glob
import sys
from plot_utils import *
import numpy as np
import matplotlib
import matplotlib.pyplot as plt
from matplotlib.pyplot import figure
figure(num=None, figsize=(10, 10), dpi=160, facecolor='w', edgecolor='k')

font = {'size': 10}
matplotlib.rc('font', **font)
path = './res/'
task = config.dataset + '-' + config.model
train_data = config.train_n
test_data = config.test_n
queries = config.query_n
decisions = config.decision_n
paths = []
colors = ['b', 'r', 'y', 'g', 'c']
# list all activelearning methods you want to plot
methods = ['random', 'uncer_y', 'decision_ig']#, 'eig']
paths_y = ['-' + name + '-' + str(train_data) + '-' + str(test_data) + '-' +
           str(decisions) + '-' + str(queries) for name in methods]
models = ['RANDOM', 'UNCER', 'DECISION IG', 'EIG']
y_args = ['acc']
names = ["ENTROPY"]
plot_args = y_args
paths.extend(paths_y)
args = len(plot_args)
for i in range(len(paths)):
    files = glob.glob(path + task + paths[i] + '*')
    files = sorted(files)
    metric = []
    for fi in files:
        with open(fi, 'rb') as f:
            x = pickle.load(f)
        try:
            metric.append(np.array(x[sys.argv[1]]))
        except:
            print("Invalid argument, try acc, or dent")
            break
    metric = np.array(metric)
    t = np.arange(metric.shape[1])
    res = mean_conf(metric)
    shadedplot(t, res, label=models[i], color=colors[i])
    #plt.plot(metric[int(sys.argv[1])],label=models[i], color=colors[i])
    #plt.plot(np.median(metric,axis=0),label=models[i], color=colors[i])
plt.legend(loc='upper center', bbox_to_anchor=(0.5, 1.1),
          fancybox=True, shadow=True, ncol=3)
plt.xlabel('QUERY')
plt.ylabel('ACCURACY')
#plt.tight_layout()
plt.savefig('./plots/entropy', bbox_inches = "tight")
plt.show()
